...
export function TransactionsPreview({ transactions }: TransactionsPreviewProps) {
  return (
    <section className="rounded-2xl bg-card px-6 py-6">
      <h2 className="text-preset-2 text-foreground">Transactions</h2>
        ...
        {transactions.map((tx, i) => {
          ...
          return (
            <li
              ...
              className="flex items-center gap-4 border-b border-subtle py-4 last:border-b-0"
            >
              <TransactionAvatar tx={tx} />
              <div className="min-w-0 flex-1">
                <p className="text-preset-4-bold text-foreground truncate">
                  {tx.name}
                </p>
                <div className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-0 text-preset-5 text-muted">
              ...
              <p
                className={`text-preset-4-bold shrink-0 ${
                  isPositive ? "text-secondary-green" : "text-foreground"
                }`}
              >
                {isPositive ? "+" : ""}
